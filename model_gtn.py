...
class GCN(nn.Module):
    ...
    def forward(self, A, X, target, num_nodes=None, eval=False, node_labels=None):
        ...
        #A = self.normalization(A, num_nodes)
        for i in range(len(A)):
            edge_index, edge_weight = A[i]
            # add dimension to edge_weight
...
class GTN(nn.Module):
    ...
    def make_embedding(self, A, X, target, num_nodes=None, eval=True, node_labels=None):
        batch_size = X.size(0)
        if num_nodes is None:
            num_nodes = self.num_nodes
        Ws = []
        ...
        X_ = X_.view(batch_size, -1, X_.size(1))
        return X_, Ws
